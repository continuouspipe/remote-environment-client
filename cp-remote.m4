...
# ARG_OPTIONAL_SINGLE([namespace], n, [Connect to a different ContinuousPipe environment])
# ARG_POSITIONAL_SINGLE([command], [Available commands], )
# ARG_POSITIONAL_SINGLE([container], [The container to use for watch, bash, resync and fetch commands (optional)], [''])
# ARG_HELP([Hello])
# ARGBASH_GO
...
case $_arg_command in
    ...
    watch)
        validate_settings
        container_specified "$_arg_container"
        watch "$(container "$_arg_container")"
        ;;
    build)
        validate_settings
        build
    ...
    bash)
        validate_settings
        container_specified "$_arg_container"
        bash "$(container "$_arg_container")"
        ;;
    ssh)
        validate_settings
        container_specified "$_arg_container"
        ssh "$(container "$_arg_container")"
        ;;
    destroy)
        validate_settings
        delete_remote
    ...
    resync)
        validate_settings
        container_specified "$_arg_container"
        resync "$(container "$_arg_container")"
        ;;
    fetch)
        validate_settings
        container_specified "$_arg_container"
        fetch "$(container "$_arg_container")"
        ;;
    forward)
        container_specified "$_arg_container"
        validate_settings
        port_forward "$_arg_container" "$3" "$4"
        ;;
    checkversion)
        check_version
        ;;
